...
def build_dataset(is_train, test_mode, args):
    ...
    if args.data_set in ['Kinetics', 'Kinetics_sparse', 'mitv1_sparse']:
        ...
        if mode == 'train':
            if not args.train_anno_path:
                raise ValueError("Please provide train_anno_path for HMDB51 dataset.")
            anno_path = args.train_anno_path
        elif mode == 'test':
            if not args.test_anno_path:
                raise ValueError("Please provide test_anno_path for HMDB51 dataset.")
        ...
        if args.include_negative_category and is_train and not args.train_multilabel:
            args.nb_classes += 1
